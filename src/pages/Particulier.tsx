...
import { EcodexLogo } from '../components/EcodexLogo'
...
function DPEBtn({ d, active, onClick, disabled = false, tip }: { d: string; active: boolean; onClick: () => void; disabled?: boolean; tip?: string | null }) {
  ...
  const btn = (
      <button onClick={disabled ? undefined : onClick} style={{
        ...
        background: DPE_COLORS[d], color: '#fff', fontWeight: 800, fontSize: 12,
  ...
  return (
    <div style={{ display: 'flex', gap: 3, alignItems: 'center', padding: '3px 0', borderBottom: '1px solid #f3f4f6', flexWrap: 'wrap' }}>
      ...
      <input type="text" value={t.label} onChange={e => onChange(i, 'label', e.target.value)}
        placeholder="Usage" style={{ padding: '5px', borderRadius: 5, border: '1px solid #d1d5db', fontSize: 14, width: 76 }} />
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '12px 10px', border: `2px solid ${accent}22`, flex: 1, minWidth: 290 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8, paddingBottom: 7, borderBottom: `2px solid ${accent}22` }}>
        ...
        <h3 style={{ margin: 0, fontSize: 15, fontWeight: 800, color: '#1f2937' }}>{title}</h3>
      </div>
      <SL color={accent} icon="🏠" label="Logement" />
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        <div><FL>Surface</FL>{areaReadOnly
          ? <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, padding: '7px 8px', borderRadius: 7, border: '1px solid #e5e7eb', background: '#f9fafb', fontSize: 14, color: '#6b7280', width: 68 }}>{sc.area} <span style={{ fontSize: 12 }}>m²</span></span>
          : <NI value={sc.area} onChange={v => setSc({ ...sc, area: v })} suffix="m²" step={5} w={68} />
        }</div>
        {!isTarget ? (
        ...
        <div style={{ marginTop: 6 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '2px 10px', marginTop: 2 }}>
            {RENOVATION_WORKS.map(w => (
              <label key={w.id} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 13, cursor: 'pointer', padding: '2px 0' }}>
                ...
                <span>{w.label}</span>
                <span style={{ fontSize: 11, color: '#9ca3af' }}>(-{Math.round(w.reduction * 100)}%, {w.costPerM2}€/m²)</span>
              </label>
            ))}
          </div>
        </div>
      ...
      {showWarn && !isTarget && ['D', 'E', 'F', 'G'].includes(sc.dpe) && (
        <div style={{ marginTop: 4, padding: '4px 7px', borderRadius: 5, background: '#fef3c7', border: '1px solid #f59e0b', fontSize: 12, color: '#92400e', lineHeight: 1.3 }}>
      ...
      <button onClick={() => setSc({ ...sc, transports: [...sc.transports, { mode: 'bike', km: 1000, label: '', occ: 1 }] })}
        style={{ marginTop: 4, padding: '3px 9px', borderRadius: 5, border: `1px dashed ${accent}66`, background: `${accent}08`, color: accent, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
        + Mode de transport
      </button>
    </div>
  )
...
function PSel({ onSelect, activeId }: { onSelect: (p: typeof PROFILES[0]) => void; activeId: string | null }) {
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#6b7280', marginBottom: 4, textTransform: 'uppercase' }}>Profil type (pré-rempli, ajustable)</div>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '12px 10px', border: '2px solid #e5e7eb' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: '#1f2937' }}>📊 Résultats</h3>
      ...
      {isCO2 ? (
              ...
              <div style={{ display: 'flex', borderRadius: 7, overflow: 'hidden', height: 54 }}>
                ...
                <div style={{ flex: Math.max(rdPct, 3), background: 'linear-gradient(135deg,#059669,#10b981)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                  <div style={{ textAlign: 'center' }}>
                    ...
                    {!redSmall && <div style={{ fontSize: 11, opacity: 0.85 }}>soit {roundTen(cur.totalCO2 - tgt.totalCO2).toLocaleString('fr-FR')} kgCO₂eq/an évités</div>}
                  </div>
                </div>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#6b7280', marginTop: 2, padding: '0 2px' }}>
                <span>restant</span>
        ...
        return (
          <div style={{ position: 'relative', cursor: 'help', marginBottom: 8 }} onMouseEnter={(e) => { const t = e.currentTarget.querySelector('[data-tip]') as HTMLElement; if (t) t.style.display = 'block' }} onMouseLeave={(e) => { const t = e.currentTarget.querySelector('[data-tip]') as HTMLElement; if (t) t.style.display = 'none' }}>
            <div style={{ display: 'flex', borderRadius: 7, overflow: 'hidden', height: 54 }}>
              ...
              <div style={{ flex: Math.max(absCR, 3), background: sv > 0 ? 'linear-gradient(135deg,#2563eb,#3b82f6)' : '#f87171', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                <div style={{ textAlign: 'center' }}>
                  ...
                  {!ecoSmall && <div style={{ fontSize: 11, opacity: 0.85 }}>{roundTen(cur.totalCost).toLocaleString('fr-FR')} → {roundTen(tgt.totalCost).toLocaleString('fr-FR')} € <span style={{ opacity: 0.7 }}>ⓘ</span></div>}
                </div>
              </div>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#6b7280', marginTop: 2, padding: '0 2px' }}>
            ...
            <div data-tip="" style={{ display: 'none', position: 'absolute', top: '100%', left: '50%', transform: 'translateX(-50%)', background: '#1f2937', color: '#fff', padding: '6px 10px', borderRadius: 7, fontSize: 12, lineHeight: 1.4, width: 280, zIndex: 50, boxShadow: '0 4px 12px rgba(0,0,0,0.25)', pointerEvents: 'none', textAlign: 'left' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {([
          ...
          return (
            <div key={l}>
              <div style={{ fontSize: 11, fontWeight: 700, color: cs[0], marginBottom: 3, textTransform: 'uppercase' }}>{l} — {roundTen(total).toLocaleString('fr-FR')} {sfx}/an</div>
...
function BizCase({ curSc, tgtSc, curR, tgtR }: { curSc: Scenario; tgtSc: Scenario; curR: AnnualResult; tgtR: AnnualResult }) {
  ...
  const tdS: React.CSSProperties = { padding: '4px 6px', borderBottom: '1px solid #f3f4f6', fontSize: 12, color: '#374151' }
  const thS: React.CSSProperties = { padding: '4px 6px', borderBottom: '1px solid #e5e7eb', fontSize: 12, fontWeight: 600, color: '#1f2937', textAlign: 'left' }

  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '12px 10px', border: '2px solid #e5e7eb' }}>
      <h3 style={{ margin: '0 0 8px', fontSize: 16, fontWeight: 800, color: '#1f2937' }}>💰 Business Case</h3>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8, marginBottom: 8 }}>
        <div style={{ padding: 8, borderRadius: 7, background: '#f9fafb' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#374151', marginBottom: 6 }}>⚙️ Paramètres</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 }}>
        <div><FL>Durée</FL><NI value={years} onChange={v => setYears(Math.max(3, Math.min(20, v)))} suffix="ans" step={1} w={48} /></div>
        <div><FL>Aides <Tip interactive align="left" below text={<>
          ...
          </>}><span style={{ opacity: 0.5, cursor: 'help' }}>ⓘ</span></Tip></FL><NI value={aidPct} onChange={v => setAidPct(Math.max(0, Math.min(90, v)))} suffix="%" step={5} w={54} /></div>
        <div><FL>Inflation fossiles <Tip align="left" below text={<>
          ...
          </>}><span style={{ opacity: 0.5, cursor: 'help' }}>ⓘ</span></Tip></FL><NI value={fInfl} onChange={v => setFInfl(Math.max(0, Math.min(15, v)))} suffix="%/an" step={1} w={54} /></div>
        <div><FL>Inflation élec <Tip align="left" below text={<>
          ...
          </>}><span style={{ opacity: 0.5, cursor: 'help' }}>ⓘ</span></Tip></FL><NI value={eInfl} onChange={v => setEInfl(Math.max(0, Math.min(10, v)))} suffix="%/an" step={1} w={54} /></div>
          </div>
        </div>
        <div style={{ padding: '8px 10px', borderRadius: 7, background: '#eff6ff', border: '1px solid #bfdbfe', fontSize: 12, color: '#1e40af', lineHeight: 1.5 }}>
        ...
        <div style={{ padding: 8, borderRadius: 7, background: '#fef2f2' }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#ef4444', marginBottom: 4 }}>🏗️ Investissement année 1</div>
          <table style={{ borderCollapse: 'collapse' }}>
            <tbody>
              ...
              {inv.vehicleCost > 0 && <tr><td style={tdS}><Tip text="Coût total d'achat des véhicules du scénario cible." align="left">Véhicule(s) cible <span style={{ opacity: 0.5, cursor: 'help' }}>ⓘ</span></Tip></td><td style={{ ...tdS, textAlign: 'right', fontWeight: 600, paddingLeft: 24 }}>{roundTen(inv.vehicleCost).toLocaleString('fr-FR')} €</td></tr>}
              {inv.vehicleReprise > 0 && <tr><td style={{ ...tdS, color: '#059669' }}><Tip text="Estimation de la valeur de reprise/revente des véhicules actuels (30% de la valeur neuve)." align="left">Reprise véhicule(s) actuel(s) <span style={{ opacity: 0.5, cursor: 'help' }}>ⓘ</span></Tip></td><td style={{ ...tdS, textAlign: 'right', color: '#059669', fontWeight: 600, paddingLeft: 24 }}>-{roundTen(inv.vehicleReprise).toLocaleString('fr-FR')} €</td></tr>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 5, marginBottom: 10 }}>
        <Tip text={(() => {
          ...
          <div style={{ padding: 6, borderRadius: 7, background: '#f0fdf4', textAlign: 'center', cursor: 'help' }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: '#059669' }}>{roundTen(cB - tB) > 0 ? '+' : ''}{roundTen(cB - tB).toLocaleString('fr-FR')} €</div>
            <div style={{ fontSize: 10, color: '#6b7280' }}>éco./an (an 1) <span style={{ opacity: 0.5 }}>ⓘ</span></div>
          </div>
        </Tip>
        <Tip text="Année à laquelle le coût cumulé du scénario cible (investissement + fonctionnement) devient inférieur au coût cumulé du scénario actuel. À partir de cette année, vous êtes « gagnant ».">
          <div style={{ padding: 6, borderRadius: 7, background: bk ? '#dbeafe' : '#fef3c7', textAlign: 'center', cursor: 'help' }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: bk ? '#2563eb' : '#d97706' }}>{bk ? `An ${bk.year}` : `>${years} ans`}</div>
            <div style={{ fontSize: 10, color: '#6b7280' }}>bascule <span style={{ opacity: 0.5 }}>ⓘ</span></div>
          </div>
        </Tip>
        <Tip text={`Gain (ou perte) net à ${years} ans : différence entre le coût cumulé du scénario actuel et celui du scénario cible, investissement inclus. Tient compte de l'inflation différenciée fossiles/électricité. Ce montant est conservateur : il ne prend pas en compte la plus-value immobilière liée à un meilleur DPE, qui peut représenter plusieurs % du prix du bien en zone tendue.`}>
          <div style={{ padding: 6, borderRadius: 7, background: gain >= 0 ? '#f0fdf4' : '#fef2f2', textAlign: 'center', cursor: 'help' }}>
            <div style={{ fontSize: 18, fontWeight: 900, color: gain >= 0 ? '#059669' : '#ef4444' }}>{gain >= 0 ? '+' : ''}{roundTen(gain).toLocaleString('fr-FR')} €</div>
            <div style={{ fontSize: 10, color: '#6b7280' }}>net à {years} ans <span style={{ opacity: 0.5 }}>ⓘ</span></div>
          </div>
        </Tip>
      </div>
      <div style={{ height: 220, marginBottom: 6 }}>
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 5, right: 8, left: 8, bottom: 5 }}>
            ...
            <RTooltip formatter={(v: number) => `${v.toLocaleString('fr-FR')} €`} labelFormatter={(l: string) => `Année ${l}`} contentStyle={{ fontSize: 12 }} />
      ...
      <div style={{ padding: '6px 10px', borderRadius: 7, background: '#fefce8', border: '1px solid #fde68a', fontSize: 11, color: '#92400e', lineHeight: 1.4 }}>
...
function Meth() {
  ...
  const h3s: React.CSSProperties = { margin: '12px 0 4px', fontSize: 13, fontWeight: 800, color: '#1f2937' }
  const ps: React.CSSProperties = { fontSize: 12, color: '#4b5563', lineHeight: 1.4, margin: '0 0 5px' }

  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: '12px 10px', border: '2px solid #e5e7eb' }}>
      <h3 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 800, color: '#1f2937' }}>📐 Méthodologie</h3>
      <div style={{ padding: '8px 10px', borderRadius: 7, background: '#eff6ff', border: '1px solid #bfdbfe', marginBottom: 10, fontSize: 12, color: '#1e40af', lineHeight: 1.5 }}>
      ...
      <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 4px', lineHeight: 1.4 }}>Périmètre FE : combustion + amont énergie</p>
      ...
      <p style={{ fontSize: 12, color: '#6b7280', margin: '0 0 4px', lineHeight: 1.4 }}>Périmètre FE : combustion + amont énergie + fabrication véhicule (+ traînées de condensation pour l'avion)</p>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 7, background: '#f6f6f6', border: '1px solid #e5e7eb', marginTop: 6 }}>
        ...
        <div style={{ fontSize: 12, color: '#374151', lineHeight: 1.4 }}>
          Cet outil est <strong>100% open source</strong> — code et méthodologie vérifiables. <a href="https://github.com/guicol57/electrification-score" target="_blank" rel="noopener noreferrer" style={{ color: '#2563eb', fontWeight: 600, textDecoration: 'none' }}>Voir sur GitHub</a>
        </div>
      </div>
    </div>
  )
...
export default function ParticulierPage() {
  ...
  useEffect(() => {
    // Fetch visit count for display (hit tracking moved to router level)
  ...
  return (
    <div className="min-h-screen font-sans">
      <div className="sticky top-0 z-20 bg-white/95 backdrop-blur border-b border-gray-200 px-2.5 pt-2.5 pb-2">
        <div style={{ position: 'relative', marginBottom: 8, maxWidth: 600, margin: '0 auto 8px' }}>
          ...
          <div style={{ textAlign: 'center' }}>
            <div className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-amber-50 border border-amber-400 text-[11px] font-semibold text-amber-800 mb-1.5">
            ...
            <p className="text-[13px] text-gray-500 m-0">Particulier · Logement + Mobilité</p>
          </div>
        </div>

        <div className="flex justify-center gap-0.5 flex-wrap">
          {navButtons.map(t => (
            <button type="button" key={t.id} onClick={t.onClick} className={`px-2.5 py-1 rounded text-[12px] font-bold border-none cursor-pointer transition-all ${isActive(t.id) ? 'bg-gray-800 text-white' : 'bg-gray-200 text-gray-500'}`}>
              {t.l}
            </button>
          ))}
        </div>
        ...
        {tab === 'home' && (
          <>
            <section id="scenarios" className="scroll-mt-32">
              <div style={{ padding: '8px 10px', borderRadius: 7, background: '#f0fdf4', border: '1px solid #bbf7d0', marginBottom: 8, fontSize: 12, color: '#166534', lineHeight: 1.5 }}>
